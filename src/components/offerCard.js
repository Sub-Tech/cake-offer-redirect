...
export default function OfferCard({...props}) {
    const pixelImpressionNodeRef = useRef(null);
    const [adReady, setAdReady] = useState(false);
    // make sure your key on the outer loop is offer_id
    const ad = props.ad;
    const id = ad?.offer_id

    useEffect(() => {
        if (adReady) return;
      pixelImpressionNodeRef.current = ad.nodePixelElement;
        setAdReady(true);
    }, [id]);
    ...
    function renderOffer () {
        ...
        return (
          <div>
              <div dangerouslySetInnerHTML={{__html: pixelImpressionNodeRef.current.outerHTML}}></div>
              <a
                href={ad.url}
                onClick={ad.onOfferClickEvent}
                target="_blank"
                rel="noopener"
              >
